...
from pidman.pid.ark_utils import normalize_ark, invalid_qualifier_characters, qualifier_allowed_characters

class TargetInlineForm(ModelForm):
    class Meta:
        model = Target
        can_delete = False  #known bug prevents this from remove delete option from inline
                            #will force by removing delete option from admin template
...
class TargetInline(admin.TabularInline):
    ...
    template = 'admin/target_inline.xhtml'
...
class DomainChoiceField(ModelChoiceField):
    ...
    def _choices(self):
         if self.cached is None:
			self.cached = list(self.generate_choices())	
         else:
			self.cached
...
class PidAdminForm(ModelForm):
    domain = DomainChoiceField(queryset=Domain.objects.filter(parent=None), cache_choices=True)    
    #domain = DomainChoiceField(queryset=Domain.objects.filter(parent=None))
    class Meta:
        model = Pid
